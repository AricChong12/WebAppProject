...
<body>
    <div id="wrapper">
        ...
        <div id="content-container">
            ...
            // echo "<br>";
            $sql = "SELECT APPLICATION_ID, BusinessName, Date, BusinessLocation FROM business_application WHERE Status='pending' AND Type='register'";
            $result = mysqli_query($conn, $sql);

            if (mysqli_num_rows($result) > 0) {
            
                while($row = mysqli_fetch_assoc($result)) {
                 echo '<span>ID: <a href="RegistView.php?id=' . $row["APPLICATION_ID"] . '">' . $row["APPLICATION_ID"] . '</a></span>' .
                 '<form method="POST" action="RegistAction.php" style="display:inline;">
